...
use crate::{Payload, PortInfo, SerialPortList, StateWrapper};
...
#[tauri::command]
pub fn stop_serial_task(state: tauri::State<StateWrapper>) -> Result<String, String> {
...
#[tauri::command]
pub fn get_port(state: tauri::State<StateWrapper>) -> Result<SerialPortList, String> {
    let portlist = tokio_serial::available_ports();
    match portlist {
        Ok(ports) => {
            let payload = SerialPortList {
                port_vec: ports.into_iter().map(|p|PortInfo{
                    port:p.port_name.clone(),
                    info:match p.port_type {
                        SerialPortType::UsbPort(info) => info.product.unwrap_or("Unknown Product".to_string()),
